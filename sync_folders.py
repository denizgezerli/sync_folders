...
import time
import sys
LOG = os.path.join(sys.argv[3],'log.txt')

# parse arguments
def get_args(args):
    return args[1], args[2], int(args[4])
...
def sync_folders(folder,backup,interval):
    log('Start')
    while True:
...
if __name__ == "__main__":
    folder, backup, interval = get_args(sys.argv)
    sync_folders(folder,backup,interval)
